<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-medicine-box class="title-icon"></i-ep-medicine-box>
        药品使用信息管理
      </h2>
      <p class="page-description">管理药品使用的记录和分析</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['natdss:natdssB012DrugUse:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['natdss:natdssB012DrugUse:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['natdss:natdssB012DrugUse:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['natdss:natdssB012DrugUse:export']" size="small">导出</el-button>
...
import {
  listNatdssB012DrugUse,
  getNatdssB012DrugUse,
  delNatdssB012DrugUse,
  addNatdssB012DrugUse,
  updateNatdssB012DrugUse
} from '@/api/natdss/natdssB012DrugUse';
import { NatdssB012DrugUseVO, NatdssB012DrugUseQuery, NatdssB012DrugUseForm } from '@/api/natdss/natdssB012DrugUse/types';
import { createNatdssB012DrugUseFieldConfig } from '@/utils/configs/natdss/natdssFieldConfigs';
import { createNatdssB012DrugUseSearchConfig } from '@/utils/configs/natdss/natdssSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createNatdssB012DrugUseFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createNatdssB012DrugUseSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listNatdssB012DrugUse(queryParams);
    natdssB012DrugUseList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取药品使用信息列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateNatdssB012DrugUse(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addNatdssB012DrugUse(form);
...
const handleExport = () => {
  proxy?.download(
    'system/natdssB012DrugUse/export',
    {
      ...queryParams
    },
