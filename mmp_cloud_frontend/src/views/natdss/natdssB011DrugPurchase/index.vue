<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-shopping-cart class="title-icon"></i-ep-shopping-cart>
        药品采购信息管理
      </h2>
      <p class="page-description">管理药品采购的记录和统计</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['natdss:natdssB011DrugPurchase:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['natdss:natdssB011DrugPurchase:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['natdss:natdssB011DrugPurchase:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['natdss:natdssB011DrugPurchase:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="natdssB011DrugPurchaseList"
...
import {
  listNatdssB011DrugPurchase,
  getNatdssB011DrugPurchase,
  delNatdssB011DrugPurchase,
  addNatdssB011DrugPurchase,
  updateNatdssB011DrugPurchase
} from '@/api/natdss/natdssB011DrugPurchase';
...
import { createNatdssB011DrugPurchaseFieldConfig } from '@/utils/configs/natdss/natdssFieldConfigs';
import { createNatdssB011DrugPurchaseSearchConfig } from '@/utils/configs/natdss/natdssSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createNatdssB011DrugPurchaseFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createNatdssB011DrugPurchaseSearchConfig();
...
const queryParams = reactive<NatdssB011DrugPurchaseQuery>({
  pageNum: 1,
  pageSize: 10,
  purSn: undefined,
  hosDrugCode: undefined,
...
const form = reactive<NatdssB011DrugPurchaseForm>({ ...initFormData });

const rules = {
  purSn: [{ required: true, message: '药品入库明细流水号不能为空', trigger: 'blur' }],
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listNatdssB011DrugPurchase(queryParams);
    natdssB011DrugPurchaseList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取药品采购信息列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateNatdssB011DrugPurchase(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addNatdssB011DrugPurchase(form);
...
const handleExport = () => {
  proxy?.download(
    'system/natdssB011DrugPurchase/export',
    {
      ...queryParams
    },
