<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-picture class="title-icon"></i-ep-picture>
        放射信息管理
      </h2>
      <p class="page-description">管理患者的放射检查信息</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['natdss:natdssB181RadInfo:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['natdss:natdssB181RadInfo:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['natdss:natdssB181RadInfo:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['natdss:natdssB181RadInfo:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['natdss:natdssB181RadInfo:edit']"></el-button>
            </el-tooltip>
...
<script setup name="NatdssB181RadInfo" lang="ts">
import {
  listNatdssB181RadInfo,
  getNatdssB181RadInfo,
  delNatdssB181RadInfo,
  addNatdssB181RadInfo,
  updateNatdssB181RadInfo
} from '@/api/natdss/natdssB181RadInfo';
import { NatdssB181RadInfoVO, NatdssB181RadInfoQuery, NatdssB181RadInfoForm } from '@/api/natdss/natdssB181RadInfo/types';
import { createNatdssB181RadInfoFieldConfig } from '@/utils/configs/natdss/natdssFieldConfigs';
import { createNatdssB181RadInfoSearchConfig } from '@/utils/configs/natdss/natdssSearchConfigs';
...
const natdssB181RadInfoList = ref<NatdssB181RadInfoVO[]>([]);
...
// 字段配置相关变量
const fieldConfigManager = createNatdssB181RadInfoFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createNatdssB181RadInfoSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listNatdssB181RadInfo(queryParams);
    natdssB181RadInfoList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取放射信息列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: NatdssB181RadInfoVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: NatdssB181RadInfoVO) => {
  reset();
  const _id = row?.id || ids.value[0];
  if (_id) {
    try {
      const res = await getNatdssB181RadInfo(_id);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateNatdssB181RadInfo(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addNatdssB181RadInfo(form);
...
/** 删除按钮操作 */
const handleDelete = async (row?: NatdssB181RadInfoVO) => {
  const _ids = row?.id || ids.value;
  try {
    await proxy?.$modal.confirm('是否确认删除应急演练计划编号为"' + _ids + '"的数据项？');
    await delNatdssB181RadInfo(_ids);
...
const handleExport = () => {
  proxy?.download(
    'system/natdssB181RadInfo/export',
    {
      ...queryParams
    },
    `natdssB181RadInfo_${new Date().getTime()}.xlsx`
  );
};
