<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理患者的诊断信息</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['natdss:natdssB023DiagInfo:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['natdss:natdssB023DiagInfo:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['natdss:natdssB023DiagInfo:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['natdss:natdssB023DiagInfo:export']" size="small">导出</el-button>
...
import {
  listNatdssB023DiagInfo,
  getNatdssB023DiagInfo,
  delNatdssB023DiagInfo,
  addNatdssB023DiagInfo,
  updateNatdssB023DiagInfo
} from '@/api/natdss/natdssB023DiagInfo';
import { NatdssB023DiagInfoVO, NatdssB023DiagInfoQuery, NatdssB023DiagInfoForm } from '@/api/natdss/natdssB023DiagInfo/types';
import { createNatdssB023DiagInfoFieldConfig } from '@/utils/configs/natdss/natdssFieldConfigs';
import { createNatdssB023DiagInfoSearchConfig } from '@/utils/configs/natdss/natdssSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createNatdssB023DiagInfoFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createNatdssB023DiagInfoSearchConfig();
...
const queryParams = reactive<NatdssB023DiagInfoQuery>({
  pageNum: 1,
  pageSize: 10,
  patientId: undefined,
  visitSn: undefined,
...
const form = reactive<NatdssB023DiagInfoForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listNatdssB023DiagInfo(queryParams);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateNatdssB023DiagInfo(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addNatdssB023DiagInfo(form);
...
const handleExport = () => {
  proxy?.download(
    'system/natdssB023DiagInfo/export',
    {
      ...queryParams
    },
