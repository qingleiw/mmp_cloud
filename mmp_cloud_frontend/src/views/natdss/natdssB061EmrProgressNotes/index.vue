<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理患者的病程记录信息</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['natdss:natdssB061EmrProgressNotes:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['natdss:natdssB061EmrProgressNotes:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['natdss:natdssB061EmrProgressNotes:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['natdss:natdssB061EmrProgressNotes:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="natdssB061EmrProgressNotesList"
...
import {
  listNatdssB061EmrProgressNotes,
  getNatdssB061EmrProgressNotes,
  delNatdssB061EmrProgressNotes,
  addNatdssB061EmrProgressNotes,
  updateNatdssB061EmrProgressNotes
} from '@/api/natdss/natdssB061EmrProgressNotes';
...
import { createNatdssB061EmrProgressNotesFieldConfig } from '@/utils/configs/natdss/natdssFieldConfigs';
import { createNatdssB061EmrProgressNotesSearchConfig } from '@/utils/configs/natdss/natdssSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createNatdssB061EmrProgressNotesFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createNatdssB061EmrProgressNotesSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listNatdssB061EmrProgressNotes(queryParams);
    natdssB061EmrProgressNotesList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取病程记录列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateNatdssB061EmrProgressNotes(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addNatdssB061EmrProgressNotes(form);
...
const handleExport = () => {
  proxy?.download(
    'system/natdssB061EmrProgressNotes/export',
    {
      ...queryParams
    },
