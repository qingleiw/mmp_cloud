  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qualification:qualificationRiskMonitor:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qualification:qualificationRiskMonitor:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="qualificationRiskMonitorList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qualification:qualificationRiskMonitor:edit']"></el-button>
            </el-tooltip>
...
const queryParams = reactive<QualificationRiskMonitorQuery>({
  pageNum: 1,
  pageSize: 10,
  riskNo: undefined,
  staffId: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQualificationRiskMonitor(queryParams);
    qualificationRiskMonitorList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取风险监测列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQualificationRiskMonitor(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQualificationRiskMonitor(form);
