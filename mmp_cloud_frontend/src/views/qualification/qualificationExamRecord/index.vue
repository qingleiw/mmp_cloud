  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qualification:qualificationExamRecord:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qualification:qualificationExamRecord:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="qualificationExamRecordList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qualification:qualificationExamRecord:edit']"></el-button>
            </el-tooltip>
...
import { createQualificationExamRecordFieldConfig } from '@/utils/configs/qualification/qualificationFieldConfigs';
import { createQualificationExamRecordSearchConfig } from '@/utils/configs/qualification/qualificationSearchConfigs';
...
const queryParams = reactive<QualificationExamRecordQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const form = reactive<QualificationExamRecordForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQualificationExamRecord(queryParams);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQualificationExamRecord(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQualificationExamRecord(form);
