      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qualification:qualificationCatalog:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qualification:qualificationCatalog:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qualification:qualificationCatalog:edit']"></el-button>
            </el-tooltip>
...
import { createQualificationCatalogFieldConfig } from '@/utils/configs/qualification/qualificationFieldConfigs';
import { createQualificationCatalogSearchConfig } from '@/utils/configs/qualification/qualificationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createQualificationCatalogFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createQualificationCatalogSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQualificationCatalog(queryParams);
    qualificationCatalogList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取资质目录列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQualificationCatalog(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQualificationCatalog(form);
