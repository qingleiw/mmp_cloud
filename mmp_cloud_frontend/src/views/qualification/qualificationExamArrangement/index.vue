  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qualification:qualificationExamArrangement:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qualification:qualificationExamArrangement:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="qualificationExamArrangementList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qualification:qualificationExamArrangement:edit']"></el-button>
            </el-tooltip>
...
import { QualificationExamArrangementVO, QualificationExamArrangementQuery, QualificationExamArrangementForm } from '@/api/qualification/qualificationExamArrangement/types';
import { createQualificationExamArrangementFieldConfig } from '@/utils/configs/qualification/qualificationFieldConfigs';
import { createQualificationExamArrangementSearchConfig } from '@/utils/configs/qualification/qualificationSearchConfigs';
...
const queryParams = reactive<QualificationExamArrangementQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const form = reactive<QualificationExamArrangementForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQualificationExamArrangement(queryParams);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQualificationExamArrangement(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQualificationExamArrangement(form);
