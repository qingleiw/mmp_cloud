      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['doctor:doctorAdverseRecord:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['doctor:doctorAdverseRecord:export']" size="small">导出</el-button>
            <el-button type="primary" plain icon="Upload" @click="handleImport" v-hasPermi="['doctor:doctorAdverseRecord:import']" size="small">导入</el-button>
      ...
      >
        ...
        >
          <template #default="scope">
            <!-- 日期字段格式化 -->
            <span v-if="field.prop === 'occurDate' || field.prop === 'createTime' || field.prop === 'updateTime'">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="后果" prop="consequence">
              <el-input v-model="form.consequence" placeholder="请输入后果" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="处理措施" prop="handlingMeasure">
              <el-input v-model="form.handlingMeasure" placeholder="请输入处理措施" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="处罚" prop="punishment">
              <el-input v-model="form.punishment" placeholder="请输入处罚" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
const queryParams = reactive<DoctorAdverseRecordQuery>({
  pageNum: 1,
  pageSize: 10,
  doctorId: undefined,
  recordType: undefined,
...
const form = reactive<DoctorAdverseRecordForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listDoctorAdverseRecord(queryParams);
...
const handleQuery = () => {
  // 处理daterange字段
  if (queryParams.occurDate && Array.isArray(queryParams.occurDate)) {
    queryParams.occurDateStart = queryParams.occurDate[0];
    queryParams.occurDateEnd = queryParams.occurDate[1];
  } else {
    queryParams.occurDateStart = undefined;
    queryParams.occurDateEnd = undefined;
  }
...
const submitForm = () => {
  doctorAdverseRecordFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.id) {
        await updateDoctorAdverseRecord(form).finally(() => (buttonLoading.value = false));
      } else {
        await addDoctorAdverseRecord(form).finally(() => (buttonLoading.value = false));
      }
...
const handleDelete = async (row?: DoctorAdverseRecordVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除医师不良记录编号为"' + _ids + '"的数据项？').finally(() => (loading.value = false));
