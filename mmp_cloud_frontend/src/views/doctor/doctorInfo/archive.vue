<template>
  <div class="p-2">
    ...
    <el-row :gutter="10">
        ...
        <el-card shadow="never" class="tree-card">
          <template #header>
            <div class="flex items-center justify-between">
              <span>科室人员目录</span>
              <el-button link size="small" @click="loadDeptTree">刷新</el-button>
            </div>
          </template>
          <el-tree
            ref="deptTreeRef"
      ...
      <el-col :span="18">
        ...
        <el-card v-if="currentView === 'doctor'" shadow="never">
          ...
          <!-- 资质证书信息 -->
          <div v-if="doctorCertificates.length > 0" class="mt-6">
            <el-divider>资质证书信息</el-divider>
            <div v-for="(certificate, index) in doctorCertificates" :key="index" class="mb-4">
              <el-card shadow="hover">
                <div>
                  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px">
                    <h4 style="margin: 0; font-weight: 600; color: #409eff; font-size: 16px">{{ certificate.certificateName }}</h4>
                    <el-tag type="success">{{ certificate.certificateType }}</el-tag>
                  </div>
                  <el-row :gutter="20" class="mb-2">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>证书编号：</strong>{{ certificate.certificateNo }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>发证机构：</strong>{{ certificate.issuingAuthority }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>发证日期：</strong>{{ parseTime(certificate.issueDate, '{y}-{m}-{d}') }}</p>
                    </el-col>
                  </el-row>
                  <el-row :gutter="20">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>到期日期：</strong>{{ parseTime(certificate.expiryDate, '{y}-{m}-{d}') }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>状态：</strong>{{ certificate.status }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p v-if="certificate.renewalDate" style="margin: 4px 0; color: #606266">
                        <strong>续证日期：</strong>{{ parseTime(certificate.renewalDate, '{y}-{m}-{d}') }}
                      </p>
                    </el-col>
                  </el-row>
                </div>
          ...
          <!-- 医师能力评估信息 -->
          <div v-if="doctorAbilityAssessments.length > 0" class="mt-6">
            <el-divider>医师能力评估信息</el-divider>
            <div v-for="(assessment, index) in doctorAbilityAssessments" :key="index" class="mb-4">
              <el-card shadow="hover">
                <div>
                  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px">
                    <h4 style="margin: 0; font-weight: 600; color: #409eff; font-size: 16px">{{ assessment.assessmentName }}</h4>
                    <el-tag type="warning">{{ assessment.assessmentResult }}</el-tag>
                  </div>
                  <el-row :gutter="20" class="mb-2">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>评估年度：</strong>{{ assessment.assessmentYear }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>评估周期：</strong>{{ assessment.assessmentPeriod }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>总分：</strong>{{ assessment.totalScore }}</p>
                    </el-col>
                  </el-row>
                  <el-row :gutter="20">
                    <el-col :span="8">
                    ...
                    <el-col :span="8">
                      <p v-if="assessment.nextAssessmentDate" style="margin: 4px 0; color: #606266">
                        <strong>下次评估日期：</strong>{{ parseTime(assessment.nextAssessmentDate, '{y}-{m}-{d}') }}
                      </p>
                    </el-col>
                  </el-row>
                </div>
          ...
          <!-- 医师电子化注册信息 -->
          <div v-if="doctorElectronicRegistrations.length > 0" class="mt-6">
            <el-divider>医师电子化注册信息</el-divider>
            <div v-for="(registration, index) in doctorElectronicRegistrations" :key="index" class="mb-4">
              <el-card shadow="hover">
                <div>
                  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px">
                    <h4 style="margin: 0; font-weight: 600; color: #409eff; font-size: 16px">{{ registration.registrationType }}</h4>
                    <el-tag type="info">{{ registration.registrationStatus }}</el-tag>
                  </div>
                  <el-row :gutter="20" class="mb-2">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>注册编号：</strong>{{ registration.registrationNo }}</p>
                    </el-col>
                    <el-col :span="8">
                  ...
                  <el-row :gutter="20">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>到期日期：</strong>{{ parseTime(registration.expiryDate, '{y}-{m}-{d}') }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>变更类型：</strong>{{ registration.changeType }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p v-if="registration.changeDate" style="margin: 4px 0; color: #606266">
                        <strong>变更日期：</strong>{{ parseTime(registration.changeDate, '{y}-{m}-{d}') }}
                      </p>
                    </el-col>
                  </el-row>
                </div>
          ...
          <!-- 医保医师码信息 -->
          <div v-if="doctorInsuranceCodes.length > 0" class="mt-6">
            <el-divider>医保医师码信息</el-divider>
            <div v-for="(insurance, index) in doctorInsuranceCodes" :key="index" class="mb-4">
              <el-card shadow="hover">
                <div>
                  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px">
                    <h4 style="margin: 0; font-weight: 600; color: #409eff; font-size: 16px">{{ insurance.insuranceType }}</h4>
                    <el-tag type="success">{{ insurance.status }}</el-tag>
                  </div>
                  <el-row :gutter="20" class="mb-2">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>医师码：</strong>{{ insurance.doctorCode }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>医保机构：</strong>{{ insurance.insuranceAuthority }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>生效日期：</strong>{{ parseTime(insurance.effectiveDate, '{y}-{m}-{d}') }}</p>
                    </el-col>
                  </el-row>
                  <el-row :gutter="20">
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>到期日期：</strong>{{ parseTime(insurance.expiryDate, '{y}-{m}-{d}') }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p style="margin: 4px 0; color: #606266"><strong>地区：</strong>{{ insurance.region }}</p>
                    </el-col>
                    <el-col :span="8">
                      <p v-if="insurance.renewalDate" style="margin: 4px 0; color: #606266">
                        <strong>续期日期：</strong>{{ parseTime(insurance.renewalDate, '{y}-{m}-{d}') }}
                      </p>
                    </el-col>
                  </el-row>
                </div>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="doctorInfoFormRef" :model="form" :rules="rules" label-width="80px">
...
import { listDoctorCertificate } from '@/api/system/doctorCertificate';
import { listDoctorAbilityAssessment } from '@/api/system/doctorAbilityAssessment';
import { listDoctorElectronicRegistration } from '@/api/system/doctorElectronicRegistration';
import { listDoctorInsuranceCode } from '@/api/system/doctorInsuranceCode';
...
import type { DoctorCertificateVO } from '@/api/system/doctorCertificate/types';
import type { DoctorAbilityAssessmentVO } from '@/api/system/doctorAbilityAssessment/types';
import type { DoctorElectronicRegistrationVO } from '@/api/system/doctorElectronicRegistration/types';
import type { DoctorInsuranceCodeVO } from '@/api/system/doctorInsuranceCode/types';
...
localStorage.removeItem('doctorInfo_field_config');
...
const doctorCertificates = ref<DoctorCertificateVO[]>([]);
const doctorAbilityAssessments = ref<DoctorAbilityAssessmentVO[]>([]);
const doctorElectronicRegistrations = ref<DoctorElectronicRegistrationVO[]>([]);
const doctorInsuranceCodes = ref<DoctorInsuranceCodeVO[]>([]);
...
const getList = async () => {
  ...
  try {
    ...
    // 显示更详细的错误信息
    const errorMessage = error?.response?.data?.msg || error?.message || '获取医生列表失败';
...
const loadDeptTree = async () => {
  try {
    ...
    // 显示更详细的错误信息
    const errorMessage = error?.response?.data?.msg || error?.message || '加载科室数据失败';
    proxy?.$modal.msgError(errorMessage);
  }
};
...
const loadDoctorWorkExperiences = async (doctorId: string | number) => {
  try {
    const res = await listDoctorWorkExperience({ doctorId, pageNum: 1, pageSize: 100 });
...
const loadDoctorEducations = async (doctorId: string | number) => {
  try {
    const res = await listDoctorEducation({ doctorId, pageNum: 1, pageSize: 100 });
...
const loadDoctorTrainings = async (doctorId: string | number) => {
  try {
    const res = await listDoctorTraining({ doctorId, pageNum: 1, pageSize: 100 });
...
const loadDoctorHonors = async (doctorId: string | number) => {
  try {
    const res = await listDoctorHonor({ doctorId, pageNum: 1, pageSize: 100 });
...
const loadDoctorPublications = async (doctorId: string | number) => {
  try {
    const res = await listDoctorPublication({ doctorId, pageNum: 1, pageSize: 100 });
...
const loadDoctorResearchProjects = async (doctorId: string | number) => {
  try {
    const res = await listDoctorResearchProject({ doctorId, pageNum: 1, pageSize: 100 });
...
const loadDoctorAcademicPositions = async (doctorId: string | number) => {
  try {
    const res = await listDoctorAcademicPosition({ doctorId, pageNum: 1, pageSize: 100 });
...
/** 加载医生资质证书信息 */
const loadDoctorCertificates = async (doctorId: string | number) => {
  try {
    const res = await listDoctorCertificate({ doctorId, pageNum: 1, pageSize: 100 });
    doctorCertificates.value = res.rows || [];
  } catch (error) {
    console.error('加载医生资质证书信息失败:', error);
    doctorCertificates.value = [];
  }
};

/** 加载医生能力评估信息 */
const loadDoctorAbilityAssessments = async (doctorId: string | number) => {
  try {
    const res = await listDoctorAbilityAssessment({ doctorId, pageNum: 1, pageSize: 100 });
    doctorAbilityAssessments.value = res.rows || [];
  } catch (error) {
    console.error('加载医生能力评估信息失败:', error);
    doctorAbilityAssessments.value = [];
  }
};

/** 加载医生电子化注册信息 */
const loadDoctorElectronicRegistrations = async (doctorId: string | number) => {
  try {
    const res = await listDoctorElectronicRegistration({ doctorId, pageNum: 1, pageSize: 100 });
    doctorElectronicRegistrations.value = res.rows || [];
  } catch (error) {
    console.error('加载医生电子化注册信息失败:', error);
    doctorElectronicRegistrations.value = [];
  }
};

/** 加载医生医保医师码信息 */
const loadDoctorInsuranceCodes = async (doctorId: string | number) => {
  try {
    const res = await listDoctorInsuranceCode({ doctorId, pageNum: 1, pageSize: 100 });
    doctorInsuranceCodes.value = res.rows || [];
  } catch (error) {
    console.error('加载医生医保医师码信息失败:', error);
    doctorInsuranceCodes.value = [];
  }
};
...
const handleNodeClick = async (data: any) => {
  ...
  if (data.type === 'dept') {
    ...
    await loadDoctorCertificates(data.doctorData.id);
    await loadDoctorAbilityAssessments(data.doctorData.id);
    await loadDoctorElectronicRegistrations(data.doctorData.id);
    await loadDoctorInsuranceCodes(data.doctorData.id);
