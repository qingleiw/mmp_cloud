<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理辐射健康体检的结果和分析</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-title">
            ...
            <span>辐射健康体检结果列表</span>
            <el-tag type="info" size="small" class="ml-2">{{ total }} 条记录</el-tag>
          </div>
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationHealthExamResult:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationHealthExamResult:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationHealthExamResultList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationHealthExamResult:edit']"></el-button>
            </el-tooltip>
...
import { RadiationHealthExamResultVO, RadiationHealthExamResultQuery, RadiationHealthExamResultForm } from '@/api/radiation/radiationHealthExamResult/types';
import { createRadiationHealthExamResultFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationHealthExamResultSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationHealthExamResultFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationHealthExamResultSearchConfig();
...
const queryParams = reactive<RadiationHealthExamResultQuery>({
  pageNum: 1,
  pageSize: 10,
  planId: undefined,
  staffId: undefined,
...
const form = reactive<RadiationHealthExamResultForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationHealthExamResult(queryParams);
    radiationHealthExamResultList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射健康体检结果列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: RadiationHealthExamResultVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改放射健康体检结果';
    } catch (error) {
      console.error('获取放射健康体检结果详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationHealthExamResult(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationHealthExamResult(form);
