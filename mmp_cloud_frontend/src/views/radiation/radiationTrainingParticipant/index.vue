<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-avatar class="title-icon"></i-ep-avatar>
        辐射培训参与人员管理
      </h2>
      <p class="page-description">管理辐射培训的参与人员和记录</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-title">
            ...
            <span>辐射培训参与人员列表</span>
            <el-tag type="info" size="small" class="ml-2">{{ total }} 条记录</el-tag>
          </div>
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationTrainingParticipant:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['radiation:radiationTrainingParticipant:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['radiation:radiationTrainingParticipant:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationTrainingParticipant:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationTrainingParticipantList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationTrainingParticipant:edit']"></el-button>
            </el-tooltip>
...
import { RadiationTrainingParticipantVO, RadiationTrainingParticipantQuery, RadiationTrainingParticipantForm } from '@/api/radiation/radiationTrainingParticipant/types';
import { createRadiationTrainingParticipantFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationTrainingParticipantSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationTrainingParticipantFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationTrainingParticipantSearchConfig();
...
const queryParams = reactive<RadiationTrainingParticipantQuery>({
  pageNum: 1,
  pageSize: 10,
  planId: undefined,
  staffId: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationTrainingParticipant(queryParams);
    radiationTrainingParticipantList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射培训参与人员列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: RadiationTrainingParticipantVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改放射培训参与人员';
    } catch (error) {
      console.error('获取放射培训参与人员详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationTrainingParticipant(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationTrainingParticipant(form);
