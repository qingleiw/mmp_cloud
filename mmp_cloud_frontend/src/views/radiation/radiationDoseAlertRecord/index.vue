<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理辐射剂量预警的记录和处理</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-title">
            ...
            <span>辐射剂量预警记录列表</span>
            <el-tag type="info" size="small" class="ml-2">{{ total }} 条记录</el-tag>
          </div>
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationDoseAlertRecord:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['radiation:radiationDoseAlertRecord:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['radiation:radiationDoseAlertRecord:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationDoseAlertRecord:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationDoseAlertRecordList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationDoseAlertRecord:edit']"></el-button>
            </el-tooltip>
...
} from '@/api/radiation/radiationDoseAlertRecord';
import { RadiationDoseAlertRecordVO, RadiationDoseAlertRecordQuery, RadiationDoseAlertRecordForm } from '@/api/radiation/radiationDoseAlertRecord/types';
import { createRadiationDoseAlertRecordFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationDoseAlertRecordSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationDoseAlertRecordFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationDoseAlertRecordSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationDoseAlertRecord(queryParams);
    radiationDoseAlertRecordList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射剂量预警记录列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationDoseAlertRecord(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationDoseAlertRecord(form);
