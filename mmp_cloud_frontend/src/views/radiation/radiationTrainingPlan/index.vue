<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理辐射培训的计划和安排</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationTrainingPlan:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['radiation:radiationTrainingPlan:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationTrainingPlan:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationTrainingPlanList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationTrainingPlan:edit']"></el-button>
            </el-tooltip>
...
import { createRadiationTrainingPlanFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationTrainingPlanSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationTrainingPlanFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationTrainingPlanSearchConfig();
...
const queryParams = reactive<RadiationTrainingPlanQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const form = reactive<RadiationTrainingPlanForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationTrainingPlan(queryParams);
    radiationTrainingPlanList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射培训计划列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationTrainingPlan(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationTrainingPlan(form);
...
const handleDelete = async (row?: RadiationTrainingPlanVO) => {
  ...
  try {
    await proxy?.$modal.confirm('是否确认删除辐射培训计划编号为"' + _ids + '"的数据项？');
