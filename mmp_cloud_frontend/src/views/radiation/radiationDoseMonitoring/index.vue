<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-monitor class="title-icon"></i-ep-monitor>
        辐射剂量监测管理
      </h2>
      <p class="page-description">管理辐射剂量的监测和分析</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationDoseMonitoring:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationDoseMonitoring:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationDoseMonitoringList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationDoseMonitoring:edit']"></el-button>
            </el-tooltip>
...
import { createRadiationDoseMonitoringFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationDoseMonitoringSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationDoseMonitoringFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationDoseMonitoringSearchConfig();
...
const queryParams = reactive<RadiationDoseMonitoringQuery>({
  pageNum: 1,
  pageSize: 10,
  workerId: undefined,
  monitoringDate: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationDoseMonitoring(queryParams);
    radiationDoseMonitoringList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射剂量监测列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: RadiationDoseMonitoringVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改放射剂量监测';
    } catch (error) {
      console.error('获取放射剂量监测详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationDoseMonitoring(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationDoseMonitoring(form);
