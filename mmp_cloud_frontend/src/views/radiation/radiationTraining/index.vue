<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理辐射安全培训的组织和实施</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationTraining:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['radiation:radiationTraining:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationTraining:export']" size="small">导出</el-button>
...
import { createRadiationTrainingFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationTrainingSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationTrainingFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationTrainingSearchConfig();
...
const form = reactive<RadiationTrainingForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationTraining(queryParams);
    radiationTrainingList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射培训列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationTraining(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationTraining(form);
