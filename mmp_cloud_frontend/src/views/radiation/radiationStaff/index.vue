<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理辐射工作人员的信息和档案</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationStaff:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['radiation:radiationStaff:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationStaff:export']" size="small">导出</el-button>
...
import {
  ...
  delRadiationStaff,
  addRadiationStaff,
  updateRadiationStaff
} from '@/api/radiation/radiationStaff';
import { RadiationStaffVO, RadiationStaffQuery, RadiationStaffForm } from '@/api/radiation/radiationStaff/types';
import { createRadiationStaffFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationStaffSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationStaffFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationStaffSearchConfig();
...
const form = reactive<RadiationStaffForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationStaff(queryParams);
    radiationStaffList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射工作人员列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationStaff(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationStaff(form);
