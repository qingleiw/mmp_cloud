<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-bell class="title-icon"></i-ep-bell>
        辐射剂量预警管理
      </h2>
      <p class="page-description">管理辐射剂量的预警规则和阈值</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationDoseAlert:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['radiation:radiationDoseAlert:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationDoseAlert:export']" size="small">导出</el-button>
...
import { createRadiationDoseAlertFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationDoseAlertSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationDoseAlertFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationDoseAlertSearchConfig();
...
const queryParams = reactive<RadiationDoseAlertQuery>({
  pageNum: 1,
  pageSize: 10,
  alertType: undefined,
  thresholdValue: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationDoseAlert(queryParams);
    radiationDoseAlertList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射剂量预警列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: RadiationDoseAlertVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改放射剂量预警';
    } catch (error) {
      console.error('获取放射剂量预警详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationDoseAlert(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationDoseAlert(form);
