<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-box class="title-icon"></i-ep-box>
        辐射设备检查管理
      </h2>
      <p class="page-description">管理辐射设备的检查和维护</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationEquipmentInspection:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationEquipmentInspection:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationEquipmentInspectionList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationEquipmentInspection:edit']"></el-button>
            </el-tooltip>
...
import { RadiationEquipmentInspectionVO, RadiationEquipmentInspectionQuery, RadiationEquipmentInspectionForm } from '@/api/radiation/radiationEquipmentInspection/types';
import { createRadiationEquipmentInspectionFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationEquipmentInspectionSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationEquipmentInspectionFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationEquipmentInspectionSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationEquipmentInspection(queryParams);
    radiationEquipmentInspectionList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射设备检查列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationEquipmentInspection(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationEquipmentInspection(form);
