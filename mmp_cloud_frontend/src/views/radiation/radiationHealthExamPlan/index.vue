<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理辐射健康体检的计划和安排</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-title">
            ...
            <span>辐射健康体检计划列表</span>
            <el-tag type="info" size="small" class="ml-2">{{ total }} 条记录</el-tag>
          </div>
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['radiation:radiationHealthExamPlan:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['radiation:radiationHealthExamPlan:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['radiation:radiationHealthExamPlan:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['radiation:radiationHealthExamPlan:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="radiationHealthExamPlanList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['radiation:radiationHealthExamPlan:edit']"></el-button>
            </el-tooltip>
...
import { createRadiationHealthExamPlanFieldConfig } from '@/utils/configs/radiation/radiationFieldConfigs';
import { createRadiationHealthExamPlanSearchConfig } from '@/utils/configs/radiation/radiationSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createRadiationHealthExamPlanFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createRadiationHealthExamPlanSearchConfig();
...
const queryParams = reactive<RadiationHealthExamPlanQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listRadiationHealthExamPlan(queryParams);
    radiationHealthExamPlanList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取辐射健康体检计划列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateRadiationHealthExamPlan(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addRadiationHealthExamPlan(form);
