      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['exam:examQuestion:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['exam:examQuestion:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['exam:examQuestion:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['exam:examQuestion:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['exam:examQuestion:edit']"></el-button>
            </el-tooltip>
...
<script setup name="ExamQuestion" lang="ts">
import {
  listExamQuestion,
  getExamQuestion,
  delExamQuestion,
  addExamQuestion,
  updateExamQuestion
} from '@/api/exam/ExamQuestion';
import { ExamQuestionVO, ExamQuestionQuery, ExamQuestionForm } from '@/api/exam/ExamQuestion/types';
import { createExamQuestionFieldConfig } from '@/utils/configs/exam/examFieldConfigs';
import { createExamQuestionSearchConfig } from '@/utils/configs/exam/examSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createExamQuestionFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createExamQuestionSearchConfig();
...
const initFormData: ExamQuestionForm = {
  id: undefined,
...
const queryParams = reactive<ExamQuestionQuery>({
  pageNum: 1,
  pageSize: 10,
  questionCode: undefined,
  qualificationId: undefined,
...
const form = reactive<ExamQuestionForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listExamQuestion(queryParams);
    examQuestionList.value = res.rows;
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: ExamQuestionVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: ExamQuestionVO) => {
  reset();
  const _id = row?.id || ids.value[0];
  if (_id) {
    try {
      const res = await getExamQuestion(_id);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateExamQuestion(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addExamQuestion(form);
...
/** 删除按钮操作 */
const handleDelete = async (row?: ExamQuestionVO) => {
  const _ids = row?.id || ids.value;
  try {
    await proxy?.$modal.confirm('是否确认删除应急演练计划编号为"' + _ids + '"的数据项？');
    await delExamQuestion(_ids);
...
const handleExport = () => {
  proxy?.download(
    'system/examQuestion/export',
    {
      ...queryParams
    },
    `examQuestion_${new Date().getTime()}.xlsx`
  );
};
