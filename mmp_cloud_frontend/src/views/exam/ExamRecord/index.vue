      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['exam:examRecord:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['exam:examRecord:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['exam:examRecord:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['exam:examRecord:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['exam:examRecord:edit']"></el-button>
            </el-tooltip>
...
<script setup name="ExamRecord" lang="ts">
import {
  listExamRecord,
  getExamRecord,
  delExamRecord,
  addExamRecord,
  updateExamRecord
} from '@/api/exam/ExamRecord';
import { ExamRecordVO, ExamRecordQuery, ExamRecordForm } from '@/api/exam/ExamRecord/types';
import { createExamRecordFieldConfig } from '@/utils/configs/exam/examFieldConfigs';
import { createExamRecordSearchConfig } from '@/utils/configs/exam/examSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createExamRecordFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createExamRecordSearchConfig();
...
const initFormData: ExamRecordForm = {
  id: undefined,
  examId: undefined,
...
const queryParams = reactive<ExamRecordQuery>({
  pageNum: 1,
  pageSize: 10,
  examId: undefined,
  examineeId: undefined,
...
const form = reactive<ExamRecordForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listExamRecord(queryParams);
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: ExamRecordVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: ExamRecordVO) => {
  reset();
  const _id = row?.id || ids.value[0];
  if (_id) {
    try {
      const res = await getExamRecord(_id);
...
/** 删除按钮操作 */
const handleDelete = async (row?: ExamRecordVO) => {
  const _ids = row?.id || ids.value;
  try {
    await proxy?.$modal.confirm('是否确认删除应急演练计划编号为"' + _ids + '"的数据项？');
    await delExamRecord(_ids);
