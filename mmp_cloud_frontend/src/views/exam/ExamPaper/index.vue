      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['exam:examPaper:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['exam:examPaper:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['exam:examPaper:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['exam:examPaper:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['exam:examPaper:edit']"></el-button>
            </el-tooltip>
...
<script setup name="ExamPaper" lang="ts">
import {
  listExamPaper,
  getExamPaper,
  delExamPaper,
...
import { ExamPaperVO, ExamPaperQuery, ExamPaperForm } from '@/api/exam/ExamPaper/types';
import { createExamPaperFieldConfig } from '@/utils/configs/exam/examFieldConfigs';
import { createExamPaperSearchConfig } from '@/utils/configs/exam/examSearchConfigs';
...
const examPaperList = ref<ExamPaperVO[]>([]);
...
// 字段配置相关变量
const fieldConfigManager = createExamPaperFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createExamPaperSearchConfig();
...
const initFormData: ExamPaperForm = {
  id: undefined,
  paperCode: undefined,
  paperName: undefined,
...
const queryParams = reactive<ExamPaperQuery>({
  pageNum: 1,
  pageSize: 10,
  paperCode: undefined,
  paperName: undefined,
...
const form = reactive<ExamPaperForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listExamPaper(queryParams);
    examPaperList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取考试试卷列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: ExamPaperVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: ExamPaperVO) => {
  reset();
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateExamPaper(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addExamPaper(form);
...
/** 删除按钮操作 */
const handleDelete = async (row?: ExamPaperVO) => {
  const _ids = row?.id || ids.value;
  try {
    await proxy?.$modal.confirm('是否确认删除应急演练计划编号为"' + _ids + '"的数据项？');
    await delExamPaper(_ids);
