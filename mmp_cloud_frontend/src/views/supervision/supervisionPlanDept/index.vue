<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导计划涉及的科室</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionPlanDept:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionPlanDept:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionPlanDept:edit']"></el-button>
            </el-tooltip>
...
import { createSupervisionPlanDeptFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionPlanDeptSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionPlanDeptFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionPlanDeptSearchConfig();
...
const form = reactive<SupervisionPlanDeptForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionPlanDept(queryParams);
    supervisionPlanDeptList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导计划科室列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionPlanDept(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionPlanDept(form);
