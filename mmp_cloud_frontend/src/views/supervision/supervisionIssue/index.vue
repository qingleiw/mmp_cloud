<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导检查发现的问题</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionIssue:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['supervision:supervisionIssue:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionIssue:export']" size="small">导出</el-button>
...
import { createSupervisionIssueFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionIssueSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionIssueFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionIssueSearchConfig();
...
const queryParams = reactive<SupervisionIssueQuery>({
  pageNum: 1,
  pageSize: 10,
  resultId: undefined,
  issueType: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionIssue(queryParams);
    supervisionIssueList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导问题列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionIssueVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取督查问题详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionIssue(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionIssue(form);
