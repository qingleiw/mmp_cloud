<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导问题的整改任务</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionRectificationTask:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionRectificationTask:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="supervisionRectificationTaskList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionRectificationTask:edit']"></el-button>
            </el-tooltip>
...
import { SupervisionRectificationTaskVO, SupervisionRectificationTaskQuery, SupervisionRectificationTaskForm } from '@/api/supervision/supervisionRectificationTask/types';
import { createSupervisionRectificationTaskFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionRectificationTaskSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionRectificationTaskFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionRectificationTaskSearchConfig();
...
const queryParams = reactive<SupervisionRectificationTaskQuery>({
  pageNum: 1,
  pageSize: 10,
  issueId: undefined,
  taskDescription: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionRectificationTask(queryParams);
    supervisionRectificationTaskList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导整改任务列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionRectificationTaskVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改督查整改任务';
    } catch (error) {
      console.error('获取督查整改任务详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionRectificationTask(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionRectificationTask(form);
