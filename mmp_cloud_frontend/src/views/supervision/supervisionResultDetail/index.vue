<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导结果的详细记录</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionResultDetail:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['supervision:supervisionResultDetail:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['supervision:supervisionResultDetail:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionResultDetail:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="supervisionResultDetailList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionResultDetail:edit']"></el-button>
            </el-tooltip>
...
import { createSupervisionResultDetailFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionResultDetailSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionResultDetailFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionResultDetailSearchConfig();
...
const queryParams = reactive<SupervisionResultDetailQuery>({
  pageNum: 1,
  pageSize: 10,
  resultId: undefined,
  indicatorId: undefined,
...
const form = reactive<SupervisionResultDetailForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionResultDetail(queryParams);
    supervisionResultDetailList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导结果明细列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionResultDetailVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改督查结果详情';
    } catch (error) {
      console.error('获取督查结果详情详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionResultDetail(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionResultDetail(form);
