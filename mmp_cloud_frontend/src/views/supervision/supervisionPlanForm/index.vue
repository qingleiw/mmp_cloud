<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导计划使用的表单</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionPlanForm:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionPlanForm:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionPlanForm:edit']"></el-button>
            </el-tooltip>
...
import { createSupervisionPlanFormFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionPlanFormSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionPlanFormFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionPlanFormSearchConfig();
...
const queryParams = reactive<SupervisionPlanFormQuery>({
  pageNum: 1,
  pageSize: 10,
  planId: undefined,
  formId: undefined,
...
const form = reactive<SupervisionPlanFormForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionPlanForm(queryParams);
    supervisionPlanFormList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导计划表单列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionPlanFormVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改督查计划表单';
    } catch (error) {
      console.error('获取督查计划表单详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionPlanForm(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionPlanForm(form);
