<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-trend-charts class="title-icon"></i-ep-trend-charts>
        督导整改进度管理
      </h2>
      <p class="page-description">管理督导问题的整改进度跟踪</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionRectificationProgress:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['supervision:supervisionRectificationProgress:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['supervision:supervisionRectificationProgress:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionRectificationProgress:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="supervisionRectificationProgressList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionRectificationProgress:edit']"></el-button>
            </el-tooltip>
...
import { SupervisionRectificationProgressVO, SupervisionRectificationProgressQuery, SupervisionRectificationProgressForm } from '@/api/supervision/supervisionRectificationProgress/types';
import { createSupervisionRectificationProgressFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionRectificationProgressSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionRectificationProgressFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionRectificationProgressSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionRectificationProgress(queryParams);
    supervisionRectificationProgressList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导整改进度列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionRectificationProgress(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionRectificationProgress(form);
