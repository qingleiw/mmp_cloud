<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导检查的计划和安排</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionPlan:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionPlan:export']" size="small">导出</el-button>
...
import { createSupervisionPlanFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionPlanSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionPlanFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionPlanSearchConfig();
...
const queryParams = reactive<SupervisionPlanQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const form = reactive<SupervisionPlanForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionPlan(queryParams);
    supervisionPlanList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导计划列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionPlan(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionPlan(form);
