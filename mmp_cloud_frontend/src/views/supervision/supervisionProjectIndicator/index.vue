<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导项目的考核指标</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionProjectIndicator:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionProjectIndicator:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="supervisionProjectIndicatorList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionProjectIndicator:edit']"></el-button>
            </el-tooltip>
...
import { SupervisionProjectIndicatorVO, SupervisionProjectIndicatorQuery, SupervisionProjectIndicatorForm } from '@/api/supervision/supervisionProjectIndicator/types';
import { createSupervisionProjectIndicatorFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionProjectIndicatorSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionProjectIndicatorFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionProjectIndicatorSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionProjectIndicator(queryParams);
    supervisionProjectIndicatorList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导项目指标列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionProjectIndicator(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionProjectIndicator(form);
