<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导检查的表单模板</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionForm:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['supervision:supervisionForm:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionForm:export']" size="small">导出</el-button>
...
import {
  ...
  delSupervisionForm,
  addSupervisionForm,
  updateSupervisionForm
} from '@/api/supervision/supervisionForm';
import { SupervisionFormVO, SupervisionFormQuery, SupervisionFormForm } from '@/api/supervision/supervisionForm/types';
import { createSupervisionFormFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionFormSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionFormFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionFormSearchConfig();
...
const queryParams = reactive<SupervisionFormQuery>({
  pageNum: 1,
  pageSize: 10,
  formCode: undefined,
  formName: undefined,
...
const form = reactive<SupervisionFormForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionForm(queryParams);
    supervisionFormList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导表单列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionFormVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取督查表单详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionForm(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionForm(form);
