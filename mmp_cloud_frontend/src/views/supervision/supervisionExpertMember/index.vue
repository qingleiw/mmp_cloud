<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理督导专家成员的信息和职责</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionExpertMember:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionExpertMember:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="supervisionExpertMemberList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionExpertMember:edit']"></el-button>
            </el-tooltip>
...
import { createSupervisionExpertMemberFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionExpertMemberSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionExpertMemberFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionExpertMemberSearchConfig();
...
const queryParams = reactive<SupervisionExpertMemberQuery>({
  pageNum: 1,
  pageSize: 10,
  groupId: undefined,
  expertName: undefined,
...
const form = reactive<SupervisionExpertMemberForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionExpertMember(queryParams);
    supervisionExpertMemberList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导专家成员列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionExpertMemberVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改督查专家成员';
    } catch (error) {
      console.error('获取督查专家成员详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionExpertMember(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionExpertMember(form);
