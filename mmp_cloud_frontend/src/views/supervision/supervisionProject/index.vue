<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-folder-opened class="title-icon"></i-ep-folder-opened>
        督导项目管理
      </h2>
      <p class="page-description">管理督导项目的定义和配置</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionProject:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionProject:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionProject:edit']"></el-button>
            </el-tooltip>
...
import { createSupervisionProjectFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionProjectSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionProjectFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionProjectSearchConfig();
...
const queryParams = reactive<SupervisionProjectQuery>({
  pageNum: 1,
  pageSize: 10,
  projectCode: undefined,
  projectName: undefined,
...
const form = reactive<SupervisionProjectForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionProject(queryParams);
    supervisionProjectList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导项目列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionProjectVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取督查项目详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionProject(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionProject(form);
