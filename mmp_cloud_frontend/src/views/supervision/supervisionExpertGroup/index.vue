<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-office-building class="title-icon"></i-ep-office-building>
        督导专家组管理
      </h2>
      <p class="page-description">管理督导专家组的组建和配置</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['supervision:supervisionExpertGroup:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['supervision:supervisionExpertGroup:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['supervision:supervisionExpertGroup:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="supervisionExpertGroupList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supervision:supervisionExpertGroup:edit']"></el-button>
            </el-tooltip>
...
import { createSupervisionExpertGroupFieldConfig } from '@/utils/configs/supervision/supervisionFieldConfigs';
import { createSupervisionExpertGroupSearchConfig } from '@/utils/configs/supervision/supervisionSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSupervisionExpertGroupFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSupervisionExpertGroupSearchConfig();
...
const queryParams = reactive<SupervisionExpertGroupQuery>({
  pageNum: 1,
  pageSize: 10,
  groupCode: undefined,
  groupName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSupervisionExpertGroup(queryParams);
    supervisionExpertGroupList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取督导专家组列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SupervisionExpertGroupVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改督查专家组';
    } catch (error) {
      console.error('获取督查专家组详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSupervisionExpertGroup(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSupervisionExpertGroup(form);
