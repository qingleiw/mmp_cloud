<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理质控指标的采集配置规则</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-title">
            ...
            <span>质控指标采集配置列表</span>
            <el-tag type="info" size="small" class="ml-2">{{ total }} 条记录</el-tag>
          </div>
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qc:qcIndicatorCollectionConfig:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qc:qcIndicatorCollectionConfig:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="qcIndicatorCollectionConfigList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qc:qcIndicatorCollectionConfig:edit']"></el-button>
            </el-tooltip>
...
import { QcIndicatorCollectionConfigVO, QcIndicatorCollectionConfigQuery, QcIndicatorCollectionConfigForm } from '@/api/qc/qcIndicatorCollectionConfig/types';
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQcIndicatorCollectionConfig(queryParams);
    qcIndicatorCollectionConfigList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取质控指标采集配置列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQcIndicatorCollectionConfig(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQcIndicatorCollectionConfig(form);
