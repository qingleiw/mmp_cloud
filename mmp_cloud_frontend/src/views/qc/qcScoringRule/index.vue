<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理质控评分的规则和标准</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qc:qcScoringRule:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['qc:qcScoringRule:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['qc:qcScoringRule:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qc:qcScoringRule:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qc:qcScoringRule:edit']"></el-button>
            </el-tooltip>
...
import {
  ...
  addQcScoringRule,
  updateQcScoringRule
} from '@/api/qc/qcScoringRule';
import { QcScoringRuleVO, QcScoringRuleQuery, QcScoringRuleForm } from '@/api/qc/qcScoringRule/types';
import { createQcScoringRuleFieldConfig } from '@/utils/configs/qc/qcFieldConfigs';
import { createQcScoringRuleSearchConfig } from '@/utils/configs/qc/qcSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createQcScoringRuleFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createQcScoringRuleSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQcScoringRule(queryParams);
    qcScoringRuleList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取质控评分规则列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQcScoringRule(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQcScoringRule(form);
