<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理质控专科的分类和层级</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qc:qcSpecialtyCategory:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qc:qcSpecialtyCategory:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qc:qcSpecialtyCategory:edit']"></el-button>
            </el-tooltip>
...
import { createQcSpecialtyCategoryFieldConfig } from '@/utils/configs/qc/qcFieldConfigs';
import { createQcSpecialtyCategorySearchConfig } from '@/utils/configs/qc/qcSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createQcSpecialtyCategoryFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createQcSpecialtyCategorySearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQcSpecialtyCategory(queryParams);
    qcSpecialtyCategoryList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取质控专科分类列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQcSpecialtyCategory(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQcSpecialtyCategory(form);
