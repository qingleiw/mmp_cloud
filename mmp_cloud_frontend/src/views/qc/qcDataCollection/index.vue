<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理质控数据的采集和录入</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qc:qcDataCollection:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['qc:qcDataCollection:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['qc:qcDataCollection:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qc:qcDataCollection:export']" size="small">导出</el-button>
...
// 字段配置相关变量
const fieldConfigManager = createQcDataCollectionFieldConfig();
...
const queryParams = reactive<QcDataCollectionQuery>({
  pageNum: 1,
  pageSize: 10,
  indicatorId: undefined,
  collectionPeriod: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQcDataCollection(queryParams);
    qcDataCollectionList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取质控数据采集列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: QcDataCollectionVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改质控数据采集';
    } catch (error) {
      console.error('获取质控数据采集详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQcDataCollection(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQcDataCollection(form);
