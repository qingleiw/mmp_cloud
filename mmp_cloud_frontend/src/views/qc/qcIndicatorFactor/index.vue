<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-histogram class="title-icon"></i-ep-histogram>
        质控指标因素管理
      </h2>
      <p class="page-description">管理质控指标的影响因素和权重</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['qc:qcIndicatorFactor:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['qc:qcIndicatorFactor:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['qc:qcIndicatorFactor:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['qc:qcIndicatorFactor:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['qc:qcIndicatorFactor:edit']"></el-button>
            </el-tooltip>
...
import { createQcIndicatorFactorFieldConfig } from '@/utils/configs/qc/qcFieldConfigs';
import { createQcIndicatorFactorSearchConfig } from '@/utils/configs/qc/qcSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createQcIndicatorFactorFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createQcIndicatorFactorSearchConfig();
...
const queryParams = reactive<QcIndicatorFactorQuery>({
  pageNum: 1,
  pageSize: 10,
  indicatorId: undefined,
  factorType: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listQcIndicatorFactor(queryParams);
    qcIndicatorFactorList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取质控指标因素列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: QcIndicatorFactorVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改质控指标因素';
    } catch (error) {
      console.error('获取质控指标因素详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateQcIndicatorFactor(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addQcIndicatorFactor(form);
