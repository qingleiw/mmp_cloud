<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理肿瘤质控的报告生成</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['tumorqc:tumorQcReport:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['tumorqc:tumorQcReport:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['tumorqc:tumorQcReport:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['tumorqc:tumorQcReport:export']" size="small">导出</el-button>
...
import {
  ...
  addTumorQcReport,
  updateTumorQcReport
} from '@/api/tumorqc/tumorQcReport';
import { TumorQcReportVO, TumorQcReportQuery, TumorQcReportForm } from '@/api/tumorqc/tumorQcReport/types';
import { createTumorQcReportFieldConfig } from '@/utils/configs/tumorqc/tumorqcFieldConfigs';
import { createTumorQcReportSearchConfig } from '@/utils/configs/tumorqc/tumorqcSearchConfigs';
...
const queryParams = reactive<TumorQcReportQuery>({
  pageNum: 1,
  pageSize: 10,
  reportCode: undefined,
  reportTitle: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listTumorQcReport(queryParams);
...
const handleUpdate = async (row?: TumorQcReportVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取肿瘤质控报告详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateTumorQcReport(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addTumorQcReport(form);
