<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理肿瘤质控的结果和评价</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['tumorqc:tumorQcResult:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['tumorqc:tumorQcResult:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['tumorqc:tumorQcResult:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['tumorqc:tumorQcResult:export']" size="small">导出</el-button>
...
import {
  ...
  addTumorQcResult,
  updateTumorQcResult
} from '@/api/tumorqc/tumorQcResult';
import { TumorQcResultVO, TumorQcResultQuery, TumorQcResultForm } from '@/api/tumorqc/tumorQcResult/types';
import { createTumorQcResultFieldConfig } from '@/utils/configs/tumorqc/tumorqcFieldConfigs';
import { createTumorQcResultSearchConfig } from '@/utils/configs/tumorqc/tumorqcSearchConfigs';
...
const queryParams = reactive<TumorQcResultQuery>({
  pageNum: 1,
  pageSize: 10,
  resultCode: undefined,
  planId: undefined,
...
const form = reactive<TumorQcResultForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listTumorQcResult(queryParams);
...
const handleUpdate = async (row?: TumorQcResultVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取肿瘤质控结果详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateTumorQcResult(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addTumorQcResult(form);
