<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理肿瘤质控检查的执行和记录</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['tumorqc:tumorQcInspection:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['tumorqc:tumorQcInspection:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['tumorqc:tumorQcInspection:export']" size="small">导出</el-button>
...
import { createTumorQcInspectionFieldConfig } from '@/utils/configs/tumorqc/tumorqcFieldConfigs';
import { createTumorQcInspectionSearchConfig } from '@/utils/configs/tumorqc/tumorqcSearchConfigs';
...
const queryParams = reactive<TumorQcInspectionQuery>({
  pageNum: 1,
  pageSize: 10,
  inspectionCode: undefined,
  planId: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listTumorQcInspection(queryParams);
...
const handleUpdate = async (row?: TumorQcInspectionVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取肿瘤质控检查详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateTumorQcInspection(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addTumorQcInspection(form);
