<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理肿瘤质控指标的配置规则</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-title">
            ...
            <span>肿瘤质控指标配置列表</span>
            <el-tag type="info" size="small" class="ml-2">{{ total }} 条记录</el-tag>
          </div>
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['tumorqc:tumorQcIndicatorConfig:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['tumorqc:tumorQcIndicatorConfig:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="tumorQcIndicatorConfigList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['tumorqc:tumorQcIndicatorConfig:edit']"></el-button>
            </el-tooltip>
...
import { createTumorQcIndicatorConfigFieldConfig } from '@/utils/configs/tumorqc/tumorqcFieldConfigs';
import { createTumorQcIndicatorConfigSearchConfig } from '@/utils/configs/tumorqc/tumorqcSearchConfigs';
...
const queryParams = reactive<TumorQcIndicatorConfigQuery>({
  pageNum: 1,
  pageSize: 10,
  indicatorCode: undefined,
  indicatorName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listTumorQcIndicatorConfig(queryParams);
    tumorQcIndicatorConfigList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取肿瘤质控指标配置列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: TumorQcIndicatorConfigVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取肿瘤质控指标配置详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateTumorQcIndicatorConfig(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addTumorQcIndicatorConfig(form);
