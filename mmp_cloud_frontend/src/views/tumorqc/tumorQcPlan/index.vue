<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理肿瘤质控的计划和安排</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['tumorqc:tumorQcPlan:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['tumorqc:tumorQcPlan:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['tumorqc:tumorQcPlan:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['tumorqc:tumorQcPlan:export']" size="small">导出</el-button>
...
import {
  ...
  addTumorQcPlan,
  updateTumorQcPlan
} from '@/api/tumorqc/tumorQcPlan';
import { TumorQcPlanVO, TumorQcPlanQuery, TumorQcPlanForm } from '@/api/tumorqc/tumorQcPlan/types';
import { createTumorQcPlanFieldConfig } from '@/utils/configs/tumorqc/tumorqcFieldConfigs';
import { createTumorQcPlanSearchConfig } from '@/utils/configs/tumorqc/tumorqcSearchConfigs';
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listTumorQcPlan(queryParams);
    tumorQcPlanList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取肿瘤质控计划列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateTumorQcPlan(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addTumorQcPlan(form);
