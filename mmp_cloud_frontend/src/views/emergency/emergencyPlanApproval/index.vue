<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急预案的审批流程和记录</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyPlanApproval:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyPlanApproval:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="emergencyPlanApprovalList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['emergency:emergencyPlanApproval:edit']"></el-button>
            </el-tooltip>
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyPlanApprovalFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyPlanApprovalSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyPlanApproval(queryParams);
    emergencyPlanApprovalList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急预案审批列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyPlanApproval(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyPlanApproval(form);
