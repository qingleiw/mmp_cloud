<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急响应团队的组建和配置</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyTeam:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['emergency:emergencyTeam:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyTeam:export']" size="small">导出</el-button>
...
import {
  ...
  addEmergencyTeam,
  updateEmergencyTeam
} from '@/api/emergency/emergencyTeam';
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyTeamFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyTeamSearchConfig();
...
const queryParams = reactive<EmergencyTeamQuery>({
  pageNum: 1,
  pageSize: 10,
  teamCode: undefined,
  teamName: undefined,
...
const form = reactive<EmergencyTeamForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyTeam(queryParams);
    emergencyTeamList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急团队列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: EmergencyTeamVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取应急团队详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyTeam(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyTeam(form);
