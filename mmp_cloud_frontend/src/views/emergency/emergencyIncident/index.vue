        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['emergency:emergencyIncident:edit']"></el-button>
            </el-tooltip>
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyIncident(queryParams);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyIncident(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyIncident(form);
