<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急预案的制定和维护</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyPlan:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['emergency:emergencyPlan:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyPlan:export']" size="small">导出</el-button>
...
import {
  ...
  addEmergencyPlan,
  updateEmergencyPlan
} from '@/api/emergency/emergencyPlan';
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyPlanFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyPlanSearchConfig();
...
const queryParams = reactive<EmergencyPlanQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyPlan(queryParams);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyPlan(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyPlan(form);
