<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急演练计划的创建、执行和评估等全流程</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyDrillPlan:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyDrillPlan:export']" size="small">导出</el-button>
...
import { createEmergencyDrillPlanFieldConfig } from '@/utils/configs/emergency/emergencyFieldConfigs';
import { createEmergencyDrillPlanSearchConfig } from '@/utils/configs/emergency/emergencySearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyDrillPlanFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyDrillPlanSearchConfig();
...
const queryParams = reactive<EmergencyDrillPlanQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyDrillPlan(queryParams);
    emergencyDrillPlanList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急演练计划列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyDrillPlan(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyDrillPlan(form);
