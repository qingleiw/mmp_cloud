    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyTeamMember:export']"
              >导出</el-button
            >
          </el-col>
          <el-col :span="1.5">
...
// 初始化动态配置
const fieldConfig = createEmergencyTeamMemberFieldConfig();
const searchConfig = createEmergencyTeamMemberSearchConfig();

const initFormData: EmergencyTeamMemberForm = fieldConfig.getDefaultFormData();
...
const submitForm = async () => {
  ...
  try {
    if (form.value.id) {
      await updateEmergencyTeamMember(form.value);
    } else {
      await addEmergencyTeamMember(form.value);
    }
