<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急团队成员信息和职责</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyTeamMember:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyTeamMember:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['emergency:emergencyTeamMember:edit']"></el-button>
            </el-tooltip>
...
import { createEmergencyTeamMemberFieldConfig } from '@/utils/configs/emergency/emergencyFieldConfigs';
import { createEmergencyTeamMemberSearchConfig } from '@/utils/configs/emergency/emergencySearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyTeamMemberFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyTeamMemberSearchConfig();
...
const queryParams = reactive<EmergencyTeamMemberQuery>({
  pageNum: 1,
  pageSize: 10,
  teamId: undefined,
  memberName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyTeamMember(queryParams);
    emergencyTeamMemberList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急团队成员列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: EmergencyTeamMemberVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改应急团队成员';
    } catch (error) {
      console.error('获取应急团队成员详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyTeamMember(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyTeamMember(form);
