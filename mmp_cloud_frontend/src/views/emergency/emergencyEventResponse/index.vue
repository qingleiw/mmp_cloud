<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急事件的响应措施和处理流程</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyEventResponse:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyEventResponse:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="emergencyEventResponseList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['emergency:emergencyEventResponse:edit']"></el-button>
            </el-tooltip>
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyEventResponseFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyEventResponseSearchConfig();
...
const queryParams = reactive<EmergencyEventResponseQuery>({
  pageNum: 1,
  pageSize: 10,
  eventId: undefined,
  responseTeam: undefined,
...
const form = reactive<EmergencyEventResponseForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyEventResponse(queryParams);
    emergencyEventResponseList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急事件响应列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: EmergencyEventResponseVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改应急事件响应';
    } catch (error) {
      console.error('获取应急事件响应详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyEventResponse(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyEventResponse(form);
