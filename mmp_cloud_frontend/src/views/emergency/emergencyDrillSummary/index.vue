<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急演练的总结和改进建议</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyDrillSummary:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['emergency:emergencyDrillSummary:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyDrillSummary:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="emergencyDrillSummaryList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['emergency:emergencyDrillSummary:edit']"></el-button>
            </el-tooltip>
...
<script setup name="EmergencyDrillSummary" lang="ts">
import {
  listEmergencyDrillSummary,
  getEmergencyDrillSummary,
  delEmergencyDrillSummary,
  addEmergencyDrillSummary,
...
const emergencyDrillSummaryList = ref<EmergencyDrillSummaryVO[]>([]);
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyDrillSummaryFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyDrillSummarySearchConfig();
...
const queryParams = reactive<EmergencyDrillSummaryQuery>({
  pageNum: 1,
  pageSize: 10,
  planId: undefined,
  actualStartTime: undefined,
...
const form = reactive<EmergencyDrillSummaryForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyDrillSummary(queryParams);
    emergencyDrillSummaryList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急演练总结列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: EmergencyDrillSummaryVO[]) => {
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = '添加应急演练总结';
};

/** 修改按钮操作 */
const handleUpdate = async (row?: EmergencyDrillSummaryVO) => {
  reset();
  const _id = row?.id || ids.value[0];
  if (_id) {
    try {
      const res = await getEmergencyDrillSummary(_id);
      Object.assign(form, res.data);
      dialog.visible = true;
      dialog.title = '修改应急演练总结';
    } catch (error) {
      console.error('获取应急演练总结详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyDrillSummary(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyDrillSummary(form);
...
/** 删除按钮操作 */
const handleDelete = async (row?: EmergencyDrillSummaryVO) => {
  const _ids = row?.id || ids.value;
  try {
    await proxy?.$modal.confirm('是否确认删除应急演练计划编号为"' + _ids + '"的数据项？');
    await delEmergencyDrillSummary(_ids);
...
const handleExport = () => {
  proxy?.download(
    'system/emergencyDrillSummary/export',
    {
      ...queryParams
    },
    `emergencyDrillSummary_${new Date().getTime()}.xlsx`
  );
};
