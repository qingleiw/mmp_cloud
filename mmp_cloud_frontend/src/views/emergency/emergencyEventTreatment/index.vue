<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理应急事件的处置过程和结果记录</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['emergency:emergencyEventTreatment:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['emergency:emergencyEventTreatment:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="emergencyEventTreatmentList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['emergency:emergencyEventTreatment:edit']"></el-button>
            </el-tooltip>
...
// 字段配置相关变量
const fieldConfigManager = createEmergencyEventTreatmentFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createEmergencyEventTreatmentSearchConfig();
...
const queryParams = reactive<EmergencyEventTreatmentQuery>({
  pageNum: 1,
  pageSize: 10,
  eventId: undefined,
  patientId: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listEmergencyEventTreatment(queryParams);
    emergencyEventTreatmentList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取应急事件处置列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: EmergencyEventTreatmentVO) => {
  ...
  if (_id) {
    try {
      ...
      dialog.visible = true;
      dialog.title = '修改应急事件处置';
    } catch (error) {
      console.error('获取应急事件处置详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateEmergencyEventTreatment(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addEmergencyEventTreatment(form);
