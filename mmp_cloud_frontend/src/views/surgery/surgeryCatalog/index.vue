<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-menu class="title-icon"></i-ep-menu>
        手术目录管理
      </h2>
      <p class="page-description">管理手术项目的目录和分类</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['surgery:surgeryCatalog:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['surgery:surgeryCatalog:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['surgery:surgeryCatalog:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['surgery:surgeryCatalog:export']" size="small">导出</el-button>
...
import {
  ...
  delSurgeryCatalog,
  addSurgeryCatalog,
  updateSurgeryCatalog
} from '@/api/surgery/surgeryCatalog';
import { SurgeryCatalogVO, SurgeryCatalogQuery, SurgeryCatalogForm } from '@/api/surgery/surgeryCatalog/types';
import { createSurgeryCatalogFieldConfig } from '@/utils/configs/surgery/surgeryFieldConfigs';
import { createSurgeryCatalogSearchConfig } from '@/utils/configs/surgery/surgerySearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createSurgeryCatalogFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createSurgeryCatalogSearchConfig();
...
const queryParams = reactive<SurgeryCatalogQuery>({
  pageNum: 1,
  pageSize: 10,
  additionalCode: undefined,
  detailCode: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSurgeryCatalog(queryParams);
...
const handleUpdate = async (row?: SurgeryCatalogVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取手术目录详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSurgeryCatalog(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSurgeryCatalog(form);
