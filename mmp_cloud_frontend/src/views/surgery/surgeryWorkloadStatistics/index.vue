<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理手术工作量的统计和分析</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['surgery:surgeryWorkloadStatistics:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['surgery:surgeryWorkloadStatistics:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="surgeryWorkloadStatisticsList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['surgery:surgeryWorkloadStatistics:edit']"></el-button>
            </el-tooltip>
...
import { SurgeryWorkloadStatisticsVO, SurgeryWorkloadStatisticsQuery, SurgeryWorkloadStatisticsForm } from '@/api/surgery/surgeryWorkloadStatistics/types';
import { createSurgeryWorkloadStatisticsFieldConfig } from '@/utils/configs/surgery/surgeryFieldConfigs';
import { createSurgeryWorkloadStatisticsSearchConfig } from '@/utils/configs/surgery/surgerySearchConfigs';
...
const queryParams = reactive<SurgeryWorkloadStatisticsQuery>({
  pageNum: 1,
  pageSize: 10,
  doctorId: undefined,
  doctorName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSurgeryWorkloadStatistics(queryParams);
...
const handleUpdate = async (row?: SurgeryWorkloadStatisticsVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取手术工作量统计详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSurgeryWorkloadStatistics(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSurgeryWorkloadStatistics(form);
