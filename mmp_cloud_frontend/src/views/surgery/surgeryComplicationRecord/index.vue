<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理手术并发症的记录和分析</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['surgery:surgeryComplicationRecord:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['surgery:surgeryComplicationRecord:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['surgery:surgeryComplicationRecord:edit']"></el-button>
            </el-tooltip>
...
import { SurgeryComplicationRecordVO, SurgeryComplicationRecordQuery, SurgeryComplicationRecordForm } from '@/api/surgery/surgeryComplicationRecord/types';
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSurgeryComplicationRecord(queryParams);
    surgeryComplicationRecordList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取手术并发症记录列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSurgeryComplicationRecord(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSurgeryComplicationRecord(form);
