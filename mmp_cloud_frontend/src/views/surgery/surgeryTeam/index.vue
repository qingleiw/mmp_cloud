<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理手术团队的配置和人员</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['surgery:surgeryTeam:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['surgery:surgeryTeam:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['surgery:surgeryTeam:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['surgery:surgeryTeam:export']" size="small">导出</el-button>
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['surgery:surgeryTeam:edit']"></el-button>
            </el-tooltip>
...
const queryParams = reactive<SurgeryTeamQuery>({
  pageNum: 1,
  pageSize: 10,
  surgeryRecordId: undefined,
  doctorId: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSurgeryTeam(queryParams);
    surgeryTeamList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取手术团队列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleUpdate = async (row?: SurgeryTeamVO) => {
  ...
  if (_id) {
    try {
      ...
      console.error('获取手术团队详情失败:', error);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSurgeryTeam(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSurgeryTeam(form);
