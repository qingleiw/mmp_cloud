<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      ...
      <p class="page-description">管理手术过程的记录和归档</p>
    </div>
      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['surgery:surgeryRecord:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['surgery:surgeryRecord:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['surgery:surgeryRecord:remove']"
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['surgery:surgeryRecord:export']" size="small">导出</el-button>
...
import {
  ...
  addSurgeryRecord,
  updateSurgeryRecord
} from '@/api/surgery/surgeryRecord';
...
const form = reactive<SurgeryRecordForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listSurgeryRecord(queryParams);
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateSurgeryRecord(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addSurgeryRecord(form);
