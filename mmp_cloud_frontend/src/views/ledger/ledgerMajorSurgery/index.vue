      ...
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ledger:ledgerMajorSurgery:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['ledger:ledgerMajorSurgery:edit']"
              size="small"
            >修改</el-button>
            <el-button
              type="danger"
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ledger:ledgerMajorSurgery:export']" size="small">导出</el-button>
...
import { createLedgerMajorSurgeryFieldConfig } from '@/utils/configs/ledger/ledgerFieldConfigs';
import { createLedgerMajorSurgerySearchConfig } from '@/utils/configs/ledger/ledgerSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createLedgerMajorSurgeryFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createLedgerMajorSurgerySearchConfig();
...
const queryParams = reactive<LedgerMajorSurgeryQuery>({
  pageNum: 1,
  pageSize: 10,
  patientName: undefined,
  operationDate: undefined,
...
const form = reactive<LedgerMajorSurgeryForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listLedgerMajorSurgery(queryParams);
    ledgerMajorSurgeryList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取大型手术列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateLedgerMajorSurgery(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addLedgerMajorSurgery(form);
