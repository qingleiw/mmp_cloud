  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" border :data="ledgerMajorSurgeryList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="手术日期" align="center" prop="operationDate" width="180">
          <template #default="scope">
            <span>{{ parseTime(scope.row.operationDate, '{y}-{m}-{d}') }}</span>
          </template>
        </el-table-column>
        <el-table-column label="手术科室" align="center" prop="department" />
        <el-table-column label="主刀医师" align="center" prop="surgeon" />
        <el-table-column label="手术名称" align="center" prop="operationName" />
        <el-table-column label="手术等级" align="center" prop="operationLevel" />
        <el-table-column label="术前诊断" align="center" prop="preoperativeDiagnosis" />
        <el-table-column label="术后诊断" align="center" prop="postoperativeDiagnosis" />
        <el-table-column label="手术时长(分钟)" align="center" prop="operationDuration" />
        <el-table-column label="麻醉方式" align="center" prop="anesthesiaMethod" />
        <el-table-column label="术中并发症" align="center" prop="complications" />
        <el-table-column label="手术结局" align="center" prop="outcome" />
        <el-table-column label="随访结果" align="center" prop="followUpResults" />
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="ledgerMajorSurgeryFormRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="主刀医师" prop="surgeon">
          <el-input v-model="form.surgeon" placeholder="请输入主刀医师" />
        </el-form-item>
        <el-form-item label="手术名称" prop="operationName">
...
const ledgerMajorSurgeryList = ref<LedgerMajorSurgeryVO[]>([]);
...
const ledgerMajorSurgeryFormRef = ref<ElFormInstance>();
...
const initFormData: LedgerMajorSurgeryForm = {
  id: undefined,
  patientId: undefined,
...
const getList = async () => {
  loading.value = true;
  const res = await listLedgerMajorSurgery(queryParams.value);
...
/** 提交按钮 */
const submitForm = () => {
  ledgerMajorSurgeryFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
...
const handleExport = () => {
  proxy?.download(
    'ledger/ledgerMajorSurgery/export',
    {
      ...queryParams.value
    },
