  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ledger:ledgerLongStayPatient:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ledger:ledgerLongStayPatient:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="ledgerLongStayPatientList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ledger:ledgerLongStayPatient:edit']"></el-button>
            </el-tooltip>
...
import { createLedgerLongStayPatientFieldConfig } from '@/utils/configs/ledger/ledgerFieldConfigs';
import { createLedgerLongStayPatientSearchConfig } from '@/utils/configs/ledger/ledgerSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createLedgerLongStayPatientFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createLedgerLongStayPatientSearchConfig();
...
const queryParams = reactive<LedgerLongStayPatientQuery>({
  pageNum: 1,
  pageSize: 10,
  patientName: undefined,
  admissionDate: undefined,
...
const form = reactive<LedgerLongStayPatientForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listLedgerLongStayPatient(queryParams);
    ledgerLongStayPatientList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取长期住院患者列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateLedgerLongStayPatient(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addLedgerLongStayPatient(form);
