  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ledger:ledgerPatientFullCycle:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ledger:ledgerPatientFullCycle:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="ledgerPatientFullCycleList"
...
import { createLedgerPatientFullCycleFieldConfig } from '@/utils/configs/ledger/ledgerFieldConfigs';
import { createLedgerPatientFullCycleSearchConfig } from '@/utils/configs/ledger/ledgerSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createLedgerPatientFullCycleFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createLedgerPatientFullCycleSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listLedgerPatientFullCycle(queryParams);
    ledgerPatientFullCycleList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取患者全周期列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateLedgerPatientFullCycle(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addLedgerPatientFullCycle(form);
