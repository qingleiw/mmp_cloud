<template>
  <div class="app-container">
    ...
    <div class="page-header mb-4">
      <h2 class="page-title">
        <i-ep-refresh class="title-icon"></i-ep-refresh>
        非计划再手术管理
      </h2>
      <p class="page-description">管理非计划再手术记录和分析</p>
    </div>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ledger:ledgerUnplannedReoperation:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ledger:ledgerUnplannedReoperation:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="ledgerUnplannedReoperationList"
        ...
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ledger:ledgerUnplannedReoperation:edit']"></el-button>
            </el-tooltip>
...
import { LedgerUnplannedReoperationVO, LedgerUnplannedReoperationQuery, LedgerUnplannedReoperationForm } from '@/api/ledger/ledgerUnplannedReoperation/types';
import { createLedgerUnplannedReoperationFieldConfig } from '@/utils/configs/ledger/ledgerFieldConfigs';
import { createLedgerUnplannedReoperationSearchConfig } from '@/utils/configs/ledger/ledgerSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createLedgerUnplannedReoperationFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createLedgerUnplannedReoperationSearchConfig();
...
const queryParams = reactive<LedgerUnplannedReoperationQuery>({
  pageNum: 1,
  pageSize: 10,
  patientId: undefined,
  patientName: undefined,
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listLedgerUnplannedReoperation(queryParams);
    ledgerUnplannedReoperationList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取非计划再手术列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateLedgerUnplannedReoperation(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addLedgerUnplannedReoperation(form);
...
const handleExport = () => {
  proxy?.download(
    'system/ledgerUnplannedReoperation/export',
    {
      ...queryParams
    },
