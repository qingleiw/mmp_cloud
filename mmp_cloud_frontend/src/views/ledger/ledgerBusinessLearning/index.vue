  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ledger:ledgerBusinessLearning:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ledger:ledgerBusinessLearning:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="ledgerBusinessLearningList"
...
import { createLedgerBusinessLearningFieldConfig } from '@/utils/configs/ledger/ledgerFieldConfigs';
import { createLedgerBusinessLearningSearchConfig } from '@/utils/configs/ledger/ledgerSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createLedgerBusinessLearningFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createLedgerBusinessLearningSearchConfig();
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listLedgerBusinessLearning(queryParams);
    ledgerBusinessLearningList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取业务学习列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateLedgerBusinessLearning(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addLedgerBusinessLearning(form);
...
const handleExport = () => {
  proxy?.download(
    'system/ledgerBusinessLearning/export',
    {
      ...queryParams
    },
