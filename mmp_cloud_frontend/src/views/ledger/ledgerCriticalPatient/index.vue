  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-actions">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ledger:ledgerCriticalPatient:add']" size="small">新增</el-button>
            <el-button
              type="success"
              plain
              icon="Edit"
            ...
            <el-button
              ...
              size="small"
            >删除</el-button>
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ledger:ledgerCriticalPatient:export']" size="small">导出</el-button>
      ...
      <el-table
        v-loading="loading"
        border
        :data="ledgerCriticalPatientList"
...
import { createLedgerCriticalPatientFieldConfig } from '@/utils/configs/ledger/ledgerFieldConfigs';
import { createLedgerCriticalPatientSearchConfig } from '@/utils/configs/ledger/ledgerSearchConfigs';
...
// 字段配置相关变量
const fieldConfigManager = createLedgerCriticalPatientFieldConfig();
const fieldConfigVisible = ref(false);
const searchConfigManager = createLedgerCriticalPatientSearchConfig();
...
const queryParams = reactive<LedgerCriticalPatientQuery>({
  pageNum: 1,
  pageSize: 10,
  planCode: undefined,
  planName: undefined,
...
const form = reactive<LedgerCriticalPatientForm>({ ...initFormData });
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await listLedgerCriticalPatient(queryParams);
    ledgerCriticalPatientList.value = res.rows;
    total.value = res.total;
  } catch (error) {
    console.error('获取危重患者列表失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    if (form.id) {
      await updateLedgerCriticalPatient(form);
      proxy?.$modal.msgSuccess('修改成功');
    } else {
      await addLedgerCriticalPatient(form);
...
const handleExport = () => {
  proxy?.download(
    'system/ledgerCriticalPatient/export',
    {
      ...queryParams
    },
